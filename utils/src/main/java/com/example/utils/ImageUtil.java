...
public class ImageUtil {
	...
	public static Bitmap getScaleBitmap(File file, int desiredWidth, int desiredHeight) {
		...
		if(scale < 1){
			// 缩小
			resizeBmp = scaleBitmap(resizeBmp, scale,true);
		}
	 ...
	 * @return Bitmap 新图片
	 */
	public static Bitmap getScaleBitmap(Bitmap bitmap, int desiredWidth, int desiredHeight,boolean isRecyclerOld) {
		...
		resizeBmp = scaleBitmap(bitmap, scale,isRecyclerOld);
		if (!checkBitmap(resizeBmp)) {
			return null;
		}
		//超出的裁掉
		if (resizeBmp.getWidth() > desiredWidth || resizeBmp.getHeight() > desiredHeight) {
	 ...
	 * @return Bitmap 新图片
	 */
	public static Bitmap scaleBitmap(Bitmap bitmap, float scale,boolean isRecycleOld) {
		...
		try {
			...
			if (resizeBmp != bitmap&& isRecycleOld) {
				bitmap.recycle();
			}
		}
		return resizeBmp;
